...
function getDailyPicks() {
  ...
  const playedGames = [...new Set(Object.keys(lsDB)
    .filter(k => lsDB[k] && lsDB[k].t > 0)
  ...
  if (playedGames.length > 0) {
    picked = playedGames
      .sort((a, b) => (lsDB[b.id].l || 0) - (lsDB[a.id].l || 0))
      .slice(0, 5);
  }

  if (picked.length < 5) {
    const remaining = [...G_DATA].filter(g => g && !picked.find(p => p.id === g.id));
...
function dodeLayout() {
  ...
  const trendingGames = [...G_DATA].slice().sort((a, b) => {
    const ta = lsDB[a.id] ? lsDB[a.id].t : 0;
    const tb = lsDB[b.id] ? lsDB[b.id].t : 0;
    if (tb !== ta) return tb - ta;
    return (lsDB[b.id]?.l || 0) - (lsDB[a.id]?.l || 0);
  });
  const trndHtml = trendingGames.slice(0, 5).map(g => makethecard(g, 'h')).join('');
